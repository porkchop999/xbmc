...
void CGUIDialogNumeric::FrameMove()
{
  ...
  { // format up the time
    strLabel = StringUtils::Format("%2d:%02d", m_datetime.GetHour(), m_datetime.GetMinute());
    start = m_block * 3;
    end = m_block * 3 + 2;
  }
  else if (m_mode == INPUT_TIME_SECONDS)
  { // format up the time
    strLabel =
        StringUtils::Format("%2d:%02d:%02d", m_datetime.GetHour(), m_datetime.GetMinute(), m_datetime.GetSecond());
    start = m_block * 3;
    end = m_block * 3 + 2;
  }
  else if (m_mode == INPUT_DATE)
  { // format up the date
    strLabel =
        StringUtils::Format("%2d/%2d/%4d", m_datetime.GetDay(), m_datetime.GetMonth(), m_datetime.GetYear());
    start = m_block * 3;
    end = m_block * 3 + 2;
    if (m_block == 2)
      end = m_block * 3 + 4;
...
std::string CGUIDialogNumeric::GetOutputString() const
{
  ...
  {
  case INPUT_DATE:
    return StringUtils::Format("%02i/%02i/%04i", m_datetime.GetDay(), m_datetime.GetMonth(), m_datetime.GetYear());
  case INPUT_TIME:
    return StringUtils::Format("%i:%02i", m_datetime.GetHour(), m_datetime.GetMinute());
  case INPUT_TIME_SECONDS:
    return StringUtils::Format("%i:%02i:%02i", m_datetime.GetHour(), m_datetime.GetMinute(),
