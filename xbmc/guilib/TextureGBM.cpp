...
CGBMTexture::CGBMTexture(unsigned int width, unsigned int height, unsigned int format) :
  ...
  m_buffer(new CGBMBufferObject)
{
  switch(format)
  {
  case XB_FMT_A8:
    m_fourcc = DRM_FORMAT_R8;
    break;
  case XB_FMT_RGBA8:
  ...
  CLog::Log(LOGDEBUG, "new texture size {}x{} using format {}", width, height, m_fourcc);
...
void CGBMTexture::LoadToGPU()
{
  ...
  if (IsMipmapped())
  {
    ...
    glTexParameteri(m_textureTarget, GL_TEXTURE_MIN_FILTER, mipmapFilter);
  }
  else
  {
...
void CGBMTexture::GetMemory()
{
  if (m_buffer)
  {
    if (m_buffer->CreateBufferObject(m_fourcc, m_textureWidth, m_textureHeight))
    {
