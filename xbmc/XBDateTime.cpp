...
std::string CDateTime::GetAsLocalizedTime(const std::string &format, bool withSeconds) const
{
  ...
  for (size_t i=0; i < length; ++i)
  {
    ...
    else if (c=='m') // parse minutes
    {
      ...
      std::string str;
      if (partLength==1)
        str = StringUtils::Format("%d", GetMinute());
      else
        str = StringUtils::Format("%02d", GetMinute());
    ...
    else if (c=='s') // parse seconds
    {
      ...
      if (withSeconds)
      {
        ...
        std::string str;
        if (partLength==1)
          str = StringUtils::Format("%d", GetSecond());
        else
          str = StringUtils::Format("%02d", GetSecond());
  ...
  for (size_t i = 0; i < length; ++i)
  {
    ...
    else if (c=='D' || c=='d') // parse days
    {
      ...
      if (partLength==1) // single-digit number
        str = StringUtils::Format("%d", GetDay());
      else if (partLength==2) // two-digit number
        str = StringUtils::Format("%02d", GetDay());
    ...
    else if (c=='M' || c=='m') // parse month
    {
      ...
      std::string str;
      if (partLength==1) // single-digit number
        str = StringUtils::Format("%d", GetMonth());
      else if (partLength==2) // two-digit number
        str = StringUtils::Format("%02d", GetMonth());
      else // Month string
      {
        int wmonth = GetMonth();
    ...
    else if (c=='Y' || c =='y') // parse year
    {
      ...
      // Format string with the length of the mask
      std::string str = StringUtils::Format("%d", GetYear()); // four-digit number
      if (partLength <= 2)
